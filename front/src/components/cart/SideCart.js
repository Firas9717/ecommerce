      ...
      <CartStyled onClick={e => e.stopPropagation()}>
        ...
        {cart.cartItems.length > 0 &&
          <Footer>
            ...
            {!isAuthenticated
                  ? <Link  to='/my-account/login' onClick={() => toggleSideCart()}><button style={{backgroundColor: "black"}}  className='checkout-btn'> Create an account </button></Link>
                  : <Link to='/cart' onClick={() => toggleSideCart()}><button style={{backgroundColor: "black"}} className='checkout-btn'>View Cart</button></Link>
                }
            <div>
                {!isAuthenticated
